...
class Unet_plusplus(pl.LightningModule):
    ...
    def __init__(self, name, checkpoint_path, log_path=None, n_channels=1, n_classes=1, depth=3, wf=6, padding=True, 
        ...
        self.up_path_per_layer = nn.ModuleList()
        self.up_path_exits = nn.ModuleList()
        for layer in range(self.depth-1):
            to_up_channels  = 2**(wf+1)
            prev_channels = 2**(wf+0)
            self.up_path_exits.append(UNetConvBlock(prev_channels, n_classes,  padding=padding, activation=[activation,last_activation]))
            up_path = nn.ModuleList()
            for depth_idx in range(self.depth-layer-1):
                prev_channels = 2**(wf+depth_idx)
                to_up_channels = 2**(wf+1+depth_idx)
                bridge_size = (1+layer)*prev_channels
                up_path.append(UNetUpBlock(to_up_channels, prev_channels, up_mode=up_mode,
                                                    padding=padding, batch_norm= batch_norm, activation=activation, dropout=dropout, bridge_size=bridge_size))
            self.up_path_per_layer.append(up_path)
    ...
    def forward(self, x):
        ...
        y = []
        for layer, up_list in enumerate(self.up_path_per_layer):
            last_depth = len(up_list)-1
            for depth, up in enumerate(up_list):
                last_output_n_channels = blocks[depth+1].shape[1]
                if depth<last_depth:
                    x = up(blocks[depth+1][:,int(last_output_n_channels*(1-1/(layer+1))):,:,:,:], blocks[depth]) #It is not needed the whole block, but only the last output
                    blocks[depth] = torch.cat((blocks[depth], x), dim=1) #last output is always concatenated at end of block
                elif depth==last_depth:
                    x = up(blocks[depth+1], blocks[depth]) 
                    if not torch.isfinite(x).all():
                        print('ok')
                    blocks[depth] = x
                if depth==0:
                    y.append(self.up_path_exits[layer](x))
        return y
    ...
    def training_step(self, batch, batch_nb):
        ...
        loss = torch.zeros(len(y_hat),device=x.device)
        for i, y_hat_layer in enumerate(y_hat):
            losses = self.compute_loss(y, y_hat_layer)
            loss[i] = losses.pop('loss')
        red_loss = torch.mean(loss)
    ...
    def validation_step(self, batch, batch_nb):
        ...
        y_size = y.shape[2:]
        y_hat = self.forward(x)
        val_loss = torch.zeros(len(y_hat),device=x.device)
        for i, y_hat_layer in enumerate(y_hat):
            losses = self.compute_loss(y, y_hat_layer)
            val_loss[i] = losses.pop('loss')
        val_red_loss = torch.mean(val_loss)
        real_batch_size = x.shape[0]*torch.distributed.get_world_size(torch.distributed.group.WORLD) if torch.distributed.group.WORLD is not None else x.shape[0]
        self.log("Valid/loss",             val_red_loss, on_step=False, on_epoch=True, prog_bar=False, sync_dist=True)
        self.log("Valid/focal_tversky_loss", val_loss[-1], on_step=False, on_epoch=True, prog_bar=False, sync_dist=True)
...
class UNetUpBlock(pl.LightningModule):
    def __init__(self, in_size, out_size, up_mode, batch_norm, activation, padding=1, dropout=None, bridge_size=None):
        '''
        ...
        self.conv_block = UNetConvBlock(bridge_size+out_size, out_size, padding=padding, batch_norm=batch_norm, activation=activation, dropout=dropout)
