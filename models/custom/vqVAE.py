...
class Residual(nn.Module): # Residual block for VQ-VAE
...
class ResidualStack(nn.Module): # Residual stack for VQ-VAE
...
class Encoder_more(nn.Module): # Encoder for VQ-VAE
	def __init__(self, in_channels, num_hiddens, num_residual_layers, num_residual_hiddens):
		super(Encoder_more, self).__init__()
		...
		self._conv_3 = nn.Conv3d(in_channels=num_hiddens//2,
								 ...
								 kernel_size=4,
								 stride=2, padding=1)
		self._conv_int = nn.Conv3d(in_channels=num_hiddens,
								 out_channels=num_hiddens,
								 kernel_size=3,
								 stride=1, padding=1)
...
class Decoder_more(nn.Module): # Decoder for VQ-VAE
	def __init__(self, in_channels, num_hiddens, num_residual_layers, num_residual_hiddens, n_classes):
		super(Decoder_more, self).__init__()
		...
		self._conv_trans_3 = nn.ConvTranspose3d(in_channels=num_hiddens//4, 
