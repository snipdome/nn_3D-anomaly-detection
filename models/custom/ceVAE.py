...
class ceVAE(pl.LightningModule):
	...
	def training_step(self, batch, batch_nb):
		...
		# ce feedback
		noisy_x = delete_parallelepipedic_regions( x, rnd_size=[np.amax(x.shape[2:])//16, np.amax(x.shape[2:])//4], rnd_multiplicity=[1, 3], rnd_value=0 )
		y_ce_hat = self.forward(noisy_x, no_dist=True, sample=False )
		losses = self.compute_loss(x,y_vae_hat, z_dist=z_dist, y_ce_hat= y_ce_hat)
	...
	def on_predict_epoch_end(self, outputs):
		if self.data_module.must_aggregate_patches():
			...
			processed = common.hook_ex_external_code(processing_args=processing_args,  x=x, y_hat=y_hat, rec_error=rec_error)
