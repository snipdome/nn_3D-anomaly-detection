...
class Unet_3plus(pl.LightningModule):
    ...
    def __init__(self, name, checkpoint_path, log_path=None, n_channels=1, n_classes=1, depth=3, wf=6, padding=True, 
        ...
        self.n_chann_inter_conn = 2**wf
        self.n_interm_cat = self.depth * self.n_chann_inter_conn
        propagated_vars = {'activation':activation, 'batch_norm':batch_norm, 'padding':padding, 'dropout':dropout}
        

        self.inter_conn_per_layer = nn.ModuleList() # ranges every node in the encoder
        self.down_path = nn.ModuleList()
        prev_channels = n_channels
        for i in range(depth):
            #   Encoder layers
            self.down_path.append(UNetConvBlock(prev_channels, 2**(wf+i), **propagated_vars))
            prev_channels = 2**(wf+i)

            #   Encoder-decoder inter-connections
            inter_conn = nn.ModuleList()
            for j in range(depth-i-1): # number of inter-connectors between Enc and Dec
                scale_factor = 2**(depth-i-2-j)
                inter_conn.append(UNetDownBlock(prev_channels, self.n_chann_inter_conn, is_single_conv=True, down_mode='avg', scale_factor=scale_factor, **propagated_vars))
            self.inter_conn_per_layer.append(inter_conn)
        last_depth_channels = prev_channels

        self.up_path       = nn.ModuleList()
        for i in range(depth - 1):
            #   Decoder layers
            self.up_path.append(UNetConvBlock(self.n_interm_cat, self.n_interm_cat, **propagated_vars))

        
        self.intra_conn_per_layer = nn.ModuleList()
        for inter_layer in range(depth - 1):
            #   Decoder intra-connections
            intra_conn = nn.ModuleList()
            for j in range(inter_layer+1): # number of intra-connectors between the inter_layer and previous ones
                layer_diff = inter_layer-j+1
                if j==0:
                    intra_conn.append(UNetUpBlock(last_depth_channels, self.n_chann_inter_conn, up_mode='upsample', is_single_conv=True, scale_factor=2**layer_diff, bridge_size=0, **propagated_vars))
                else:
                    intra_conn.append(UNetUpBlock(self.n_interm_cat, self.n_chann_inter_conn, up_mode='upsample', is_single_conv=True, scale_factor=2**layer_diff,  bridge_size=0,**propagated_vars))
            self.intra_conn_per_layer.append(intra_conn)

        propagated_vars['activation'] = last_activation
        propagated_vars['dropout'] = False
        # Decoder exit nodes
        self.exits = nn.ModuleList()
        for i in range(depth-1):
            if i == 0:
                self.exits.append(UNetUpBlock(last_depth_channels, self.n_channels, up_mode='upsample', bridge_size=0, is_single_conv=True,scale_factor=2**(depth-i-1), **propagated_vars))
            else:
                self.exits.append(UNetUpBlock(self.n_interm_cat, self.n_channels, up_mode='upsample', bridge_size=0, is_single_conv=True,scale_factor=2**(depth-i-1), **propagated_vars))
        self.exits.append(UNetConvBlock(self.n_interm_cat, self.n_channels, is_single_conv=True, **propagated_vars))
    ...
    def forward(self, x):
        ...
        for i, up in enumerate(self.up_path):
            ...
            for j in range(self.depth-1-i):
                inter_cat.append(self.inter_conn_per_layer[j][i](blocks[j]))
            #blocks.pop()
            
            intra_cat = []
            for k in range(i+1):
                intra_cat.append(self.intra_conn_per_layer[i][k](y[k]))
            
            x = torch.cat((*inter_cat, *intra_cat), dim=1)
            #del inter_cat, intra_cat
            y.append(up(x))
            
        #del blocks, inter_cat, intra_cat
        #torch.cuda.empty_cache()

        for i, elem in enumerate(y):
            y[i] = self.exits[i](elem)
            #assert not torch.isnan(y[i]).any() #FIXME: to remove!!
    ...
    def test_epoch_end(self, outputs):
        ...
        if processing_args is None: # FIXME : not a good way to do this
            processing_args = self.data_module.test_and_predict.get('post_processing')
...
class UNetDownBlock(pl.LightningModule):
    def __init__(self, in_size, out_size, down_mode, activation='LeakyRelu', batch_norm=False, padding=1, dropout=None, scale_factor=2, is_single_conv=False):
        super(UNetDownBlock, self).__init__()
        propagated_vars = {'in_size':in_size, 'out_size':out_size, 'activation':activation, 'batch_norm':batch_norm, 'padding':padding, 'dropout':dropout, 'is_single_conv':is_single_conv}
        self.conv_block = UNetConvBlock(**propagated_vars)
        if scale_factor != 1:
...
class UNetUpBlock(pl.LightningModule):
    def __init__(self, in_size, out_size, up_mode, activation='LeakyRelu', batch_norm=False, padding=1, dropout=None, scale_factor=2, is_single_conv=False, bridge_size=None):
        '''
        ...
        propagated_vars = {'activation':activation, 'batch_norm':batch_norm, 'padding':padding, 'dropout':dropout, 'is_single_conv':is_single_conv}
        
        if scale_factor != 1:
            if up_mode == 'upconv':
                self.up = nn.ConvTranspose3d(in_size, out_size, kernel_size=scale_factor, stride=scale_factor)
        ...
        self.conv_block = UNetConvBlock(bridge_size+out_size, out_size, **propagated_vars)
